...
namespace ProtagonistCompiler
{
    public class Parser
    {
        ...
        private int parseStatement(List<Token> tokens, int i, ParseNode parseNode)
        {
            ...
            // with a non-whitespace token, parse the statement
            // TODO: create a giant state machine that parses a statement
            return i;
        }
        ...
        private int parseCharacters(List<Token> tokens, int i, ParseTree parseTree)
        {
            ...
            if (tokens[i].type == TokenType.BRACK_OPEN)
            {
                ...
                for (int j = 0; j < characterInfo.Count - 1; j++)
                {
                    ...
                    if (checkDeclarationFinish(characterInfo, j))
                    {
                        break;
                    }
        ...
        private int parseLabels(List<Token> tokens, int i, ParseTree parseTree)
        {
            ...
            LabelDefinition label = new LabelDefinition(tokens[i].contents, new ParseNode(), LabelType.GOTO);
