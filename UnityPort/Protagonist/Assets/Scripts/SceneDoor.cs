...
public class SceneDoor : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.tag == "Player" && active && GameObject.FindGameObjectWithTag("SceneTransition") == null)
        {
            ...
            Vector2 direction = gameObject.transform.position - ScreenResolution.MapViewToWorldPoint(new Vector2(0.5f, 0.5f));
            ...
            MapPosition.GetInstance().Position += new Vector2Int(cardinal.x, -cardinal.y);
            SceneTransition trans = SceneTransitions.Transition("Slide", new TransitionTime(1f, 0.25f, 1f),
                MapPosition.GetInstance().map[MapPosition.GetInstance().Position]);
            trans.side = side;
        }
    }
}
